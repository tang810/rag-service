...
def run_pdf_workflow(test_input: InputRecord, write_pdf_db: bool = True) -> dict[str, object]:
	...
	if write_pdf_db:
		logger.info("[6/11] PDF工作: 写入 documents 表")
		...
		logger.info("[7/11] PDF工作: 从 documents 表读取 doc_id")
...
def run_chunks_workflow(doc_id: str, markdown_path: str, write_chunks_db: bool = True) -> dict[str, object]:
	logger.info("[8/11] chunks工作: 构建 chunks")
	...
	logger.info("[9/11] chunks工作: Parsed segments=%d, Generated chunks=%d", len(parsed_segments), len(chunks))

	inserted_count = 0
	chunks_db_written = False
	if write_chunks_db:
		logger.info("[10/11] chunks工作: 写入 chunks 表")
...
async def _run_embedding_stage(
	...
	logger.info("[11/11] embedding工作: 读取未向量化 chunks 并生成向量")
	...
	# Loop by window to support large documents while avoiding giant API payloads.
	while True:
...
def run_pdf_chunks_embedding_test(
	...
	print(f"embedding size: {embedding_output['embedding_size']}")
	print(f"chunks processed: {embedding_output['embedding_chunks_processed']}")
	if int(embedding_output["embedding_inserted_count"]) > 0:
...
if __name__ == "__main__":
	parser = argparse.ArgumentParser(description="End-to-end PDF + chunks + embedding integration test")
	...
	result = run_pdf_chunks_embedding_test(
